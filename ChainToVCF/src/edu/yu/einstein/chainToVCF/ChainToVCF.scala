...
/**
 * Converts a Chain file into a VCF file
 */
object ChainToVCF {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (Args.help) {
      params.setProgramName("scala -classpath ChainToVCF.jar edu.yu.einstein.chainToVCF.ChainToVCF")
      params.usage
    } else {
      val vcfWriter = new VCFWriter(Args.targetReference, Args.sourceReference)
      val chainReader = new ChainReader(Args.chainFilePath)
      val variantList = chainReader.retrieveVariants
